...
class TestAccountLockoutAudit:
    ...
    def test_account_locked_event_logged(self):
        ...
        # Check audit log
        audit = AuditLog.objects.filter(event_type=AuditEventType.ACCOUNT_LOCKED, user_id=str(self.user.id)).first()
    ...
    def test_failed_login_logged(self):
        ...
        audit = AuditLog.objects.filter(
            event_type=AuditEventType.LOGIN, user_id=str(self.user.id), success=False
        ).first()
