...
class TestSecurityWorkflows:
    ...
    def test_mfa_bypass_attempt_blocked(self):
        ...
        MFASecret.objects.create(user=self.user, secret="TESTSECRET123456", is_verified=True)

        response = self.client.post("/api/auth/login/", {"email": "security@example.com", "password": "Password@123"})
    ...
    def test_concurrent_sessions_management(self):
        ...
        for _ in range(3):
            response = self.client.post(
                "/api/auth/login/", {"email": "security@example.com", "password": "Password@123"}
            )
