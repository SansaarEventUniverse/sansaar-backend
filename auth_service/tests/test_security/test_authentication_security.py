...
class TestAuthenticationSecurity:
    ...
    def test_login_with_unverified_email(self):
        ...
        response = self.client.post("/api/auth/login/", {"email": "unverified@example.com", "password": "Password@123"})
    ...
    def test_token_manipulation_attempt(self):
        ...
        response = self.client.post(
            "/api/auth/password-change/", {"current_password": "Password@123", "new_password": "NewPassword@456"}
        )
    ...
    def test_expired_refresh_token(self):
        RefreshToken.objects.create(
            user=self.user, token="expired_token", expires_at=timezone.now() - timezone.timedelta(days=1)
        )

        response = self.client.post("/api/auth/logout/", {"refresh_token": "expired_token"})
    ...
    def test_blacklisted_token_reuse(self):
        token = RefreshToken.objects.create(
            user=self.user, token="test_token", expires_at=timezone.now() + timezone.timedelta(days=7)
        )
        token.blacklist()

        response = self.client.post("/api/auth/logout/", {"refresh_token": "test_token"})

        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "Token already blacklisted" in str(response.data)

    def test_sql_injection_attempt_in_login(self):
        response = self.client.post("/api/auth/login/", {"email": "admin' OR '1'='1", "password": "anything"})
